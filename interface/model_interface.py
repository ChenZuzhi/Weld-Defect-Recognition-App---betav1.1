...
class ModelInterface:
    ...
    def __init__(self, model):
        ...
        if model == 'Inception':
            model_file=os.getcwd()+os.path.sep+'output_graph_incep.pb'
            label_file=os.getcwd()+os.path.sep+'output_labels.txt'
            input_layer = "Mul"
            output_layer = "final_result"
            input_height = 299
            input_width = 299
        elif model == 'Mobilenet':
            model_file=os.getcwd()+os.path.sep+'output_graph_mobile.pb'
            label_file=os.getcwd()+os.path.sep+'output_labels.txt'
            input_layer = "input"
            output_layer = "final_result"
            input_height = 224
...
#        print(os.getcwd())
    ...
    def predict(self, image_data_orig, sess):
        # read raw image
        dic_path=os.getcwd()+os.path.sep+'tmp_img'
#        print(dic_path)
        if not os.path.exists(dic_path):
            os.makedirs(dic_path)
        file_path=dic_path+os.path.sep+'tmp.png'
        cv2.imwrite(file_path,image_data_orig)
...
#        time1 = time.time()
        ...
        results = sess.run(self.output_operation.outputs[0], {self.input_operation.outputs[0]: resized_image})
